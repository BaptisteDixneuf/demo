...
public class StatelessAuthenticationIntegrationTest {
	...
	public void testAdminApi_Get_Users_AsAdmin() {
		...
		assertEquals("[{\"id\":4,\"username\":\"admin\",\"expires\":0,\"roles\":[\"ADMIN\"]}"
				+ ",{\"id\":5,\"username\":\"user\",\"expires\":0,\"roles\":[\"USER\"]}]", result);
	}
	...
	public void testAdminApi_GrantRole_AsAnonymous() {
		doAnonymousExchange(HttpMethod.POST, "/admin/api/users/5/grant/role/ADMIN");
	}

	@Test(expected = HttpClientErrorException.class)
	public void testAdminApi_GrantRole_AsUser() {
		doAuthenticatedExchange("user", HttpMethod.POST, "/admin/api/users/5/grant/role/ADMIN");
	}

	@Test
	public void testAdminApi_GrantRole_AsAdmin() {
		doAuthenticatedExchange("admin", HttpMethod.POST, "/admin/api/users/5/grant/role/ADMIN");
		doAuthenticatedExchange("admin", HttpMethod.POST, "/admin/api/users/5/revoke/role/ADMIN");
	}
